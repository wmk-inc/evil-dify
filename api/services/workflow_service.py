...
def generate_nodes(tools_to_use: list[ToolApiEntity], providers_to_use: list[ToolProviderApiEntity], draft):
    ...
    for index, payload in enumerate(tools_to_use):
        ...
        params = {item.name: "" for item in parameters}
        tool_parameters = {item.name: {"type": "mixed", "value": f"{{{{#{pre_id}.text#}}}}"} for item in parameters}
        param_schemas = [p.model_dump(mode="json") for p in parameters]

        node.data = GeneralNodeData(
            type="tool",
            title=payload.label.zh_Hans or "",
            desc=payload.description.zh_Hans or "",
    ...
    end_node.data = GeneralNodeData(
        type="end",
        title="End",
        desc="",
        outputs=[OutputSelector(variable="text", value_selector=[pre_id, "text"])],
        selected=False,
    )
