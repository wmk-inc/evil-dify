...
file_exts = [".txt", ".md", ".mdx", ".markdown", ".pdf",
             ".html", ".xlsx", ".xls", ".doc", ".docx", ".csv",
...
async def run(send_message_payload):
    ...
    async with httpx.AsyncClient() as httpx_client:
        ...
        async for chunk in stream_response:
            await response_queue.put(str(chunk))
            ...
            "inputs": [
                {
                    "name": p["name"],
                    "type": "file" if any(
                        tool_parameters.get(p["name"], "").lower().endswith(ext) for ext in file_exts) else "text",
            ...
            "outputs": [
                {
                    "name": op["name"],
                    "type": "file" if any(
                        tool_parameters.get(op["name"], "").lower().endswith(ext) for ext in file_exts) else "text",
                    "value": "",
